	...
	mState(*this, nullptr, Identifier("PhaserPlugin"),
		{
			  std::make_unique<AudioParameterFloat>(IDs::speed,
												    ...
												    [](float value, int) {return static_cast<String>(round(0.069f * exp(0.04f * value) * 100.f) / 100.f); },
												    [](const String& text) {return log(100.f * text.getFloatValue()) / 0.04f; }
													 ),
			  std::make_unique<AudioParameterFloat>(IDs::wetness,
													"Wetness",
													NormalisableRange<float> (0.00f, 0.5f),
												    0.5,
												    String(),
												    AudioProcessorParameter::genericParameter,
												    [](float value, int) {return static_cast<String>(round((value * 2) * 100.f * 100.f) / 100.f); },
